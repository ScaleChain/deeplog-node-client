...
Allows you to send logs to your [deeplog](https://deeplog.ai) account
from NodeJS.
...
- [Setting Up With Deeplog Itself](#setting-up-with-deeplog-itself)
...
```javascript
const Logger = require('deeplog-node-client');

const logger = new Logger({ token: 'myAccessToken' });
...
### Required
 ...
 - **token:** String. Authorization token for the Deeplog service.
...
### Behavior
 ...
 - **maxFailedAttempts:** [Removed with new connection handling]
   The number of times to retry to reach the deeplog host in case of 
   error when connecting. Default: `15`.
 - **retryTimeout:** [Removed with new connection handling] 
   Time to wait between attempts when trying to reach the deeplog host. 
   Default: `15 * 60 * 1000`.
...
### Other
 ...
 - **port**: As above. This will default to 7701 if `secure` is false, or 7601 if
   it’s true.
...
"expand JSON" (meaning pretty print) in the options on deeplog:
...
the status on Deeplog dashboard. I will investigate this further to see if there’s
...
Deeplog client will place the transport constructor at `winston.transports`,
even if Winston itself hasn’t yet been required.

```javascript
var Logger = require('deeplog-node-client');
var winston = require('winston');

assert(winston.transports.Deeplog);
```

When adding a new Deeplog transport, the options argument passed to Winston’s
...
```javascript
winston.add(winston.transports.Deeplog, { token: myToken });
```
...
## Setting Up With Deeplog Itself

When you create an account at Logentries (just a standard signup form; there’s a
free tier), and create a new service, you can find the token you need.
