...
/**
 * Logger class that handles parsing of logs and sending logs to Deeplog.
 */
class Logger extends Writable {
      ...
      if (isSecure) {
        connection = tls.connect.apply(tls, args, () => {
          if (!connection.authorized) {
            ...
            this.emit(new DeeplogError(text.authError(errMsg)));
   ...
   * to Deeplog connection when its available
   */
  _write(ch, enc, cb) {
    this.drained = false;
  ...
  log(lvl, log) {
    ...
    if (modifiedLevel || modifiedLevel === 0) {
      ...
      if (!modifiedLevel && modifiedLevel !== 0) {
        this.emit(errorEvent, new DeeplogError(text.unknownLevel(modifiedLevel)));
        return;
      }
    ...
    if (_.isArray(modifiedLog)) {
      if (modifiedLog.length) {
        ...
        this.emit(errorEvent, new DeeplogError(text.noLogMessage()));
      }
      return;
    }
    ...
    if (_.isObject(modifiedLog)) {
      ...
      if (!modifiedLog) {
        this.emit(errorEvent, new DeeplogError(text.serializedEmpty()));
        return;
      }
  ...
  static provisionWinston(winston) {
    if (winston.transports.Deeplog) return;

    const Transport = winston.Transport;

    class DeeplogTransport extends Transport {
      constructor(opts) {
        super(opts);
        this.json = opts.json;
        this.name = 'deeplog';
        ...
        if (semver.satisfies(winston.version, '>=2.0.0')) {
          // Winston and Deeplog levels are reversed
          // ('error' level is 0 for Winston and 5 for Deeplog)
    ...
    winston.transports.Deeplog = DeeplogTransport;
  }
